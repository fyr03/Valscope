...
Vertical-oriented SQL generator for VerticalOracle.

Design goal:
- preserve result comparability between vertical S1 and S2
- expand plan diversity and query surface area
- allow vertical-specific biasing such as indexed-column preference
...
class VerticalQueryGenerator:
    def __init__(
        ...
        if not tables:
            raise ValueError("VerticalQueryGenerator: at least one table is required")
        self.tables = tables
        ...
        self._enable_known_mysql_date_index_string_eq_workaround = (
            ...
            if enable_known_mysql_date_index_string_eq_workaround is not None
            else self._dialect.optimizer_family() == 'mysql'
        )
        ...
        for t2n, t2c in other:
            ...
            if pair is None:
                if any(
                    ...
                    for c1 in t1c for c2 in t2c
                ):
                    suppressed_known_pair = True
                continue
    ...
    def _build_cte_wrapper(self) -> str:
        ...
        if random.random() < 0.35:
            ...
            if other:
                ...
                if on:
                    combined_alias_cols = outer_alias_cols + [(a2, t2c)]
                    combined_sel = self._select_list(combined_alias_cols)
                    sql = (
                        f"WITH {cte_name} AS ({inner_sql}) "
                        f"SELECT {combined_sel} "
                        f"FROM {cte_name} {cte_alias} INNER JOIN {self._qi(t2n)} {a2} ON {on}{outer_where}"
                    )
                    return self._maybe_add_order_by(sql, combined_alias_cols)
    ...
    def _select_list_from_templates(self, alias_cols: List[Tuple[str, List[Any]]],
        ...
        for idx, col in templates:
            ...
            expr = self._col_expr(alias, col, alias_cols, allow_scalar_fn, mode)
            exprs.append(self._dedupe_projected_expr(expr, col.name, used_output_names))
        return ', '.join(exprs) if exprs else '*'

    def _dedupe_projected_expr(self,
                               expr: str,
                               preferred_name: str,
                               used_output_names: Dict[str, int]) -> str:
        count = used_output_names.get(preferred_name, 0)
        used_output_names[preferred_name] = count + 1
        if count == 0:
            return expr
        alias_name = f"{preferred_name}__{count + 1}"
        return f"{expr} AS {self._qi(alias_name)}"
    ...
    def _col_expr(self, alias: str, col: Any,
        ...
        r = random.random()
        indexed = getattr(col, 'is_indexed', False)
    ...
    def _predicate(self, alias: str, col: Any,
        ...
        if dt in _TEMPORAL_FAMILY:
            other = self._pick_compatible_ref(alias_cols, col, alias)
            if indexed:
                if other and r < 0.40 and not self._is_known_mysql_date_string_equality_risk(col, other[1]):
                    return f"{ref} >= {self._qref(other[0], other[1].name)}"
                if r < 0.72:
                    return f"{ref} >= {self._lit('2023-01-01', 'DATE')}"
        ...
        if dt in _INT_FAMILY:
            ...
            if indexed:
                ...
                if r < 0.90:
                    lits = [v1] + [str(random.randint(-200, 200)) for _ in range(random.randint(1, 3))]
    ...
    def _pick_join_col(self, cols: List[Any]) -> Optional[Any]:
        ...
        return random.choices(
            safe,
            weights=[4 if getattr(c, 'is_indexed', False) else 1 for c in safe],
            k=1,
        )[0]
    ...
    def _pick_compat_col(self,
        ...
        risky = [
            c for c in cols
            if self._is_risky_cross_type_pair(ref, c)
            and (
        ...
        compat = [
            c for c in cols
            if self._pairwise_type_compatible(ref, c)
            and (
                allow_known_date_index_string_eq
                or not self._is_known_date_index_string_eq(ref, c)
            )
    ...
    def _pair_predicate_from_alias_cols(self,
        ...
        for _ in range(12):
            ...
            if not compat:
                continue
            pred = self._pair_predicate(left, self._choose_ref(compat))
            if pred:
                return pred
        return None
    ...
    def _choose_ref(self, refs: List[Tuple[str, Any]]) -> Tuple[str, Any]:
        return random.choices(
            refs,
            weights=[4 if getattr(col, 'is_indexed', False) else 1 for _, col in refs],
            k=1,
        )[0]
    ...
    def _same_comparable_type(self,
        ...
        return (
            ...
            or (
                self._is_risky_cross_type_pair(c1, c2)
                and (
                    allow_known_date_index_string_eq
                    or not self._is_known_date_index_string_eq(c1, c2)
                )
    ...
    def _maybe_add_order_by(self, sql: str,
        ...
        if random.random() >= prob:
            return sql
        if self._has_top_level_distinct(sql):
            return sql
    ...
    def _order_by_expr(self, alias_cols: List[Tuple[str, List[Any]]]) -> Optional[str]:
        ...
        if not refs:
            return None
        alias, col = self._choose_ref(refs)
        ref = self._qref(alias, col.name)
        dt = col.data_type
        r = random.random()
        if dt in _NUMERIC_FAMILY:
    ...
    def _has_top_level_distinct(self, sql: str) -> bool:
        normalized = re.sub(r'\s+', ' ', sql).strip()
        return re.match(r'^SELECT\s+(?:/\*.*?\*/\s+)?DISTINCT\b', normalized, flags=re.IGNORECASE) is not None
    ...
    def _is_known_date_index_string_eq(self, c1: Any, c2: Any) -> bool:
        if not self._enable_known_mysql_date_index_string_eq_workaround:
            return False
        left_date = c1.data_type == 'DATE' and getattr(c1, 'is_indexed', False)
        right_date = c2.data_type == 'DATE' and getattr(c2, 'is_indexed', False)
